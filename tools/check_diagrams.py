#!/usr/bin/env python3
"""Hold the diagrams in README.md to the folder they describe.

    python3 tools/check_diagrams.py            # structure, scripts, and the four statements
    python3 tools/check_diagrams.py --render   # also render both diagrams, if mermaid-cli is here
...
# maintenance scripts, and the two gates that guard this repository rather than a sheet.
NOT_A_CHECK = {"extract.py", "build_reference.py", "check_freshness.py",
               "test_docs_example.py", "check_diagrams.py"}
...
def main(argv):
    ...
    if problems:
        for p in problems:
            print(f"FAIL  {p}")
        print(f"\n{len(problems)} problem(s). A diagram that disagrees with the folder is worse "
              "than no diagram: it is read and believed.")
        return 1
    ...
    if rendered:
        print(f"  OK  {rendered}")
    print("\nThe diagrams describe the folder that is here.")
    return 0
